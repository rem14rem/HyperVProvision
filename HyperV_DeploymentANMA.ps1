...
function vmnetwork-menu
{
        ...
        $VMMServerName = "pvmmanma.hvi.brown.edu"
...
{       
        ...
        Write-Host "1. anmaBackup_2wk `n2. anmaBackup_4wk  `n3. anmaBackup_6wk `n4. anmaCustomBackup `n5. anmaAgentBackup  `n6. anmaNoBackup`n"
        $tagmenuresponse = read-host [Enter Selection]
        Switch ($tagmenuresponse) {
            "1" {$script:vmtag = "anmaBackup_2wk" }
            "2" {$script:vmtag = "anmaBackup_4wk" }
            "3" {$script:vmtag = "anmaBackup_6wk" }
            "4" {$script:vmtag = "anmaCustomBackup" }
            "5" {$script:vmtag = "anmaAgentBackup" }
            "6" {$script:vmtag = "anmaNoBackup" }
            }
...
function google-it
{
    ...
    $certPath = "C:\AdminFiles\powershell-cmenard-b4405ad18f0c.p12"
...
function hyperv-menu
{
    do
    {
        ...
        $rand_count = 1
        Write-Host "Select Host Cluster:"
        Write-Host "1. HVICANMA01 (internal) `n"
        $menuresponse = read-host [Enter Selection]
        Switch ($menuresponse) {
            "1" {$script:vmhostcluster = "hvicanma01"
                        #$HostGroup =  Get-SCVMHost -VMHostGroup Internal | sort-object -property name | Select-Object -ExpandProperty name
                        $HostGroup =  Get-SCVMHost -VMMServer $VMMServerName  | sort-object -property name | Select-Object -ExpandProperty name
...
function create-vm
{
        ...
        $ISO = Get-SCISO -VMMServer $VMMServerName  | where {$_.Name -eq "DeployBootImage.iso"}      
